...
for idx, item in enumerate(display_items):
    ...
    with col:
        with st.container(border=True):
            ...
            with col_b:
                if st.button("➕ Add", key=f"add_{item.get('id')}"):
                    st.session_state.food_cart.append(item)
                    st.session_state.cart_total += item.get('price', 0)
...
if not st.session_state.food_cart:
    ...
    for idx, item in enumerate(st.session_state.food_cart):
        ...
        with col_a:
            st.write(f"{item.get('name', 'Item')} - ₹{item.get('price', 0)}")
            ...
            if st.form_submit_button("✅ Place Order", use_container_width=True):
                if not agree_terms:
                    ...
                    if "error" in response:
                        ...
                        st.balloons()
                        st.info(f"📦 **Order ID:** {response.get('id', 'N/A')}")
...
if not orders:
    ...
    for order in orders:
        with st.container(border=True):
            ...
            with col_a:
                st.markdown(f"**Order ID:** {str(order.get('id', 'N/A'))[:12]}")
