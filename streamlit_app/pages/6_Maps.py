"""
Google Maps Integration Page - Interactive Stadium Navigation
"""
...
with tab1:
    ...
    # Get highlighted gate from user selection
    highlighted_gate = st.selectbox(
        "Select a gate to highlight:",
        ["None", "Gate A", "Gate B", "Gate C", "Gate D", "Gate E"]
    )
    
    if highlighted_gate == "None":
        highlighted_gate = None
    
    # Create and display map
    stadium_map = StadiumMapHelper.create_stadium_map(highlighted_gate=highlighted_gate)
    st_folium(stadium_map, width=1200, height=500)
...
with tab2:
    ...
    # Show gate map
    gate_map = StadiumMapHelper.create_gate_detail_map(selected_gate, gate_lat, gate_lng)
    st_folium(gate_map, width=1200, height=500)
...
with tab3:
    ...
    parking_map = StadiumMapHelper.create_parking_utilization_map(parking_utilization)
    st_folium(parking_map, width=1200, height=500)
